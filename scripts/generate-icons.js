...
// Icons we want to include from Heroicons
const iconList = [
  // Contact icons
  { name: 'phone', type: 'outline' },
  { name: 'envelope', type: 'outline' },
  { name: 'map-pin', type: 'outline' },

  // UI icons
  { name: 'users', type: 'outline' },
  { name: 'cube', type: 'outline' },
  { name: 'video-camera', type: 'outline' },
  ...
  { name: 'map', type: 'outline' }, // area knowledge
  { name: 'academic-cap', type: 'outline' }, // training
...
const customIcons = {
  ...
  youtube: `<svg fill="currentColor" viewBox="0 0 24 24">
...
async function generateIcons() {
  ...
  for (const icon of iconList) {
    try {
      ...
      if (fs.existsSync(iconPath)) {
        ...
        const key = icon.type === 'solid' ? `${icon.name}-solid` : icon.name;
        icons[key] = svgContent;
        console.log(`✓ Added ${key}`);
      } else {
        console.warn(`⚠ Icon not found: ${icon.name} (${icon.type})`);
      }
    } catch (error) {
      console.error(`✗ Error loading ${icon.name}:`, error.message);
    }
  }
  ...
  const tsContent = `// Auto-generated icon definitions
...
`;
  ...
  const outputPath = path.join(__dirname, '../src/generated/icons.ts');
  ...
  console.log(
    ...
    } icons to src/generated/icons.ts`,
  );
}
